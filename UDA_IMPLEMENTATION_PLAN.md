...
*   `UDATemplateContentSerializer` (MODIFICATO - gestirà `quiz_template` invece di `quiz`).
...
*   `UDAContentSerializer` (MODIFICATO - gestirà `quiz_template` invece di `quiz`, upload/link allegati per `ACTIVITY`, lo stato `activity_completed` e `teacher_marked_completed`).
...
*   **Azioni:**
    *   Implementare i serializer, prestando attenzione alla validazione e alla gestione dei campi relazionati e dei contenuti nested/dinamici, inclusa la relazione UDA-Corso, l'ordinamento UDA nel corso e il completamento dei contenuti UDA. [COMPLETATO]
...
*   **`udaTemplateStore.ts`**: [COMPLETATO]
    ...
    *   Actions: `fetchUdaTemplates`, `fetchUdaTemplate`, `createUdaTemplate`, `updateUdaTemplate`, `deleteUdaTemplate`, `addContentToTemplate`, `updateContentInTemplate`, `removeContentFromTemplate`.
*   **`udaStore.ts`**: [COMPLETATO]
    *   State: `udas`, `currentUda`, `loading`, `error`.
    *   Actions: `fetchUdas` (con filtri per status, e opzionalmente per `courseId`), `fetchUda`, `createUda` (da zero o da template, gestendo `courseId`, `orderInCourse` e `subjectIds` [COMPLETATO]), `updateUda` (gestendo `courseId`, `orderInCourse` e `subjectIds` [COMPLETATO]), `deleteUda`, `addContentToUda`, `updateContentInUda` (inclusa `teacher_marked_completed`), `removeContentFromUda`, `completeActivityInUda` (per `activity_completed`), `updateUdaContentTeacherCompletion`.
...
*   **Azioni:**
    ...
    *   [COMPLETATO] Aggiornare `udaStore.ts` per gestire l'associazione con i corsi, l'ordinamento e il completamento dei contenuti.
...
*   Funzioni per ogni endpoint API definito nel backend:
    ...
    *   Per Template UDA: (es. `getUdaTemplates()`, `createUdaTemplate(data)`).
    *   Per UDA: (es. `getUdas()`, `createUda(data)` - modificata per includere `courseId`, `orderInCourse` e `subjectIds` [VERIFICATO - GESTITO DA STORE], `getUdaContents(udaId)`).
    *   Per Contenuti UDA: (es. `uploadActivityAttachment(udaContentId, file)`, `updateUdaContentTeacherCompletion(udaContentId, completed)`).
...
*   **Azioni:**
    ...
    *   [COMPLETATO] Aggiungere/Modificare le funzioni di servizio per includere la gestione dei Corsi, l'associazione UDA-Corso, l'ordinamento e il completamento dei contenuti.
...
*   **Componenti Specifici per Contenuti:**
    *   `UdaContentItemRenderer.vue`: MODIFICATO. [COMPLETATO]
    *   `UdaContentLessonDisplay.vue`: [COMPLETATO]
    *   `UdaContentQuizDisplay.vue`: [COMPLETATO]
    *   `UdaContentNoteDisplay.vue`: [COMPLETATO]
    *   `UdaContentActivityDisplay.vue`: [COMPLETATO]
    *   `NoteTemplateContentDisplay.vue`: NUOVO. [COMPLETATO]
    *   `ActivityTemplateContentDisplay.vue`: NUOVO. [COMPLETATO]
*   **Componenti di Supporto:**
    *   `UdaContentEditor.vue`: [COMPLETATO]
...
*   **Azioni:**
    ...
    *   [COMPLETATO] Curare l'UI/UX per la gestione dei Corsi, l'ordinamento delle UDA nei corsi, l'associazione UDA-Corso e la marcatura del completamento dei contenuti UDA.
    ...
    *   [COMPLETATO] Migliorare/Completare `UdaContentItemRenderer.vue`.
    *   [COMPLETATO] Migliorare/Completare `UdaContentEditor.vue`.
    ...
    *   [COMPLETATO] Raffinare i componenti display specifici per contenuto.
...
*   **Test di Integrazione:**
    ...
    *   Verificare la corretta creazione/gestione delle relazioni (in particolare `UDA` <-> `Subject`) e dei contenuti nested.
*   **Strumenti:** `pytest-django`, `factory-boy`.
*   **Azioni:**
    *   Scrivere e eseguire i test backend, coprendo le nuove funzionalità dei Corsi e la gestione di materie multiple per UDA. [DA FARE]

### 4.2. Frontend (Vue.js)
*   **Test Unitari:**
    *   Testare la logica dei componenti (props, computed properties, methods), inclusi i nuovi componenti per i Corsi e le modifiche ai componenti UDA (`UdaFormView.vue`) per la selezione di materie multiple. [DA FARE]
    *   Testare le actions e mutations/getters degli store Pinia, inclusi `courseStore` e le modifiche a `udaStore` (per gestione `subjectIds`). [DA FARE]
*   **Test E2E (End-to-End):**
    *   Simulare i flussi utente completi:
        ...
        *   Aggiunta/modifica/rimozione di contenuti in una UDA.
...
*   **Azioni:**
    *   Scrivere e eseguire i test frontend, coprendo le nuove funzionalità dei Corsi, l'ordinamento, il completamento esteso e la gestione di materie multiple per UDA. [DA FARE]
...
*   Backend (Funzionalità Corsi + Modifiche UDA per materie multiple): X+A giorni [COMPLETATO]
*   Frontend (Funzionalità Corsi + Modifiche UDA per materie multiple): Y+B giorni [COMPLETATO]
*   Testing (complessivo, inclusi Corsi e materie multiple UDA): Z+C giorni [DA FARE]

(Le stime X, Y, Z, A, B, C andranno definite dal team di sviluppo)
