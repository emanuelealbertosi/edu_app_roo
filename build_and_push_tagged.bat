...
REM !!! IMPOSTA QUI L'URL CORRETTO PER IL TUO FRONTEND-LESSONS IN PRODUZIONE !!!
SET VITE_LESSONS_APP_URL_BUILD_ARG=https://www.eduapp.it/lezioni/
...
echo Building "%IMAGE_FULL_NAME%" from context "%CURRENT_BUILD_CONTEXT%" with VITE_LESSONS_APP_URL="%VITE_LESSONS_APP_URL_BUILD_ARG%"...
echo Executing: docker build -t "%IMAGE_FULL_NAME%" --build-arg VITE_LESSONS_APP_URL="%VITE_LESSONS_APP_URL_BUILD_ARG%" "%CURRENT_BUILD_CONTEXT%"
docker build -t "%IMAGE_FULL_NAME%" --build-arg VITE_LESSONS_APP_URL="%VITE_LESSONS_APP_URL_BUILD_ARG%" "%CURRENT_BUILD_CONTEXT%"
SET BUILD_ERRORLEVEL=%ERRORLEVEL%
echo DEBUG: BUILD_ERRORLEVEL catturato per %SERVICE_NAME% e': [%BUILD_ERRORLEVEL%]
IF %BUILD_ERRORLEVEL% EQU 0 (
    echo DEBUG: BUILD_ERRORLEVEL per %SERVICE_NAME% e' 0. Build OK. Procedo.
