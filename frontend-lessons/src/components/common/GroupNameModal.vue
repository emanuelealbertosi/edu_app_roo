            ...
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
              <form @submit.prevent="handleSubmit">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                      ...
                      <div class="mt-2">
                        <label for="groupName" class="block text-sm font-medium text-gray-700">Nome del Gruppo</label>
                        <input
                          ref="inputRef"
                          type="text"
                          id="groupName"
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  <button type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="$emit('close')">
                    Annulla
                  </button>
                </div>
              </form>
...
watch(() => props.open, (isOpen) => {
  if (isOpen) {
    internalName.value = props.initialName || '';
    nextTick(() => {
...
const handleSubmit = () => {
  if (internalName.value.trim()) {
    emit('confirm', internalName.value.trim());
  }
};
</script>