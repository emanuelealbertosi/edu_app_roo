  ...
  <div v-if="show" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center" @click.self="closeModal">
    <div class="relative mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
      ...
      <div class="bg-blue-600 text-white p-4 rounded-t-md -m-5 mb-6"> <!-- -m-5 mb-6 per sovrapporre padding e aggiungere margine sotto -->
        <h3 class="text-xl font-semibold text-center">Seleziona Gruppi</h3>
      </div>
      <div class="mt-3 text-center"> <!-- Rimosso mt-3 dal div esterno -->
        <div class="mt-2 px-7 py-3">
          ...
          <!-- Lista Gruppi -->
          <div v-if="filteredGroups.length > 0" class="text-left max-h-80 overflow-y-auto border border-gray-200 rounded-md p-3 space-y-2 bg-gray-50 mb-4">
             <!-- Seleziona Tutti -->
          ...
          <div v-else class="text-center py-6 text-gray-500">
            Nessun gruppo corrisponde al filtro o nessun gruppo disponibile.
          </div>
...
import type { StudentGroup } from '@/types/groups'; // Importa il tipo StudentGroup
...
const props = defineProps({
  ...
  groups: { // Modificato da students a groups
    type: Array as PropType<StudentGroup[]>, // Modificato tipo
    required: true,
  },
  initialSelectedIds: {
...
const filteredGroups = computed(() => { // Modificato da filteredStudents
  if (!filterText.value) {
    return props.groups; // Modificato da props.students
  }
  const lowerFilter = filterText.value.toLowerCase();
  return props.groups.filter(group => // Modificato da props.students
    group.name.toLowerCase().includes(lowerFilter) ||
    (group.description && group.description.toLowerCase().includes(lowerFilter))
  );
});

// Logica "Seleziona Tutti" (solo filtrati)
const allFilteredSelected = computed(() => {
  const filteredIds = filteredGroups.value.map(g => g.id); // Modificato da filteredStudents
