<template>
  <div v-if="editor" class="border border-gray-300 rounded-md">
    <div class="toolbar p-2 bg-gray-100 border-b border-gray-300 flex flex-wrap gap-1">
      ...
      <button @click="editor.chain().focus().toggleUnderline().run()" :class="{ 'is-active': editor.isActive('underline') }" class="toolbar-button">
        U
      </button>
      <button @click="editor.chain().focus().toggleBulletList().run()" :class="{ 'is-active': editor.isActive('bulletList') }" class="toolbar-button">
        Lista Punt.
      </button>
      <button @click="editor.chain().focus().toggleOrderedList().run()" :class="{ 'is-active': editor.isActive('orderedList') }" class="toolbar-button">
        Lista Num.
      </button>
      <button @click="setLink" :class="{ 'is-active': editor.isActive('link') }" class="toolbar-button">
        Link
      </button>
      <button @click="editor.chain().focus().unsetLink().run()" v-if="editor.isActive('link')" class="toolbar-button">
        Rimuovi Link
      </button>
      ...
      <div class="flex gap-1 items-center">
        ...
        ></button>
        <button @click="editor?.chain().focus().unsetColor().run()" class="toolbar-button text-xs ml-1" title="Rimuovi colore">
          &#x2715; <!-- Simbolo X -->
        </button>
      </div>
    </div>
